        ...
        private async Task<bool> Generate()
        {
            ...
            foreach (Type type in types)
            {
                if (type.GetTypeInfo().BaseType == typeof(System.Enum))
                {
                    ...
                    sb.AppendLine(@"        {");
                    sb.AppendLine(@"            Assert.True(await SetupTest(culture));");
                    sb.AppendLine(@"");
                    ...
                    foreach (FieldInfo fieldInfo in type.GetFields())
                    {
                        if (fieldInfo.FieldType.GetTypeInfo().BaseType == typeof(System.Enum))
                        {
                            ...
                            {
                                sb.AppendLine($@"                        Assert.Equal(CultureEnumsRes.{ enumName }{ fName }, retStr);");
                                sb.AppendLine(@"                        break;");
                            }
                        }
                    }
            ...
            sb.AppendLine(@"            Assert.Equal(string.Format(CultureEnumsRes._IsRequired, ""PolSourceObsInfoEnum""), retStr);");
            sb.AppendLine(@"        }");
            sb.AppendLine(@"");
            ...
            foreach (Type type in types)
            {
                if (type.GetTypeInfo().BaseType == typeof(System.Enum))
                {
                    ...
                    sb.AppendLine($@"            Assert.Equal(string.Format(CultureEnumsRes._IsRequired, ""{ enumName }""), retStr);");
                    sb.AppendLine(@"");
