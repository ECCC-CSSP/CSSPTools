        ...
        public async Task<bool> Generate()
        {
            ...
            foreach (Type type in types)
            {
                ...
                if (!ClassNotMapped)
                {
                    ...
                    sb.AppendLine($@"        public { TypeName }Controller(ICSSPCultureService CSSPCultureService, ILoggedInService LoggedInService,");
                    sb.AppendLine($@"           I{ TypeName }DBService { TypeName }DBService)");
                    sb.AppendLine(@"        {");
                    ...
                    else
                    {
                        ...
                        sb.AppendLine($@"        public async Task<ActionResult<{ TypeName }>> Get(int { TypeName.Replace("Local", "") }ID)");
                    }
                    sb.AppendLine(@"        {");
                    sb.AppendLine($@"            CSSPCultureService.SetCulture((string)RouteData.Values[""culture""]);");
                    sb.AppendLine($@"            await LoggedInService.SetLoggedInContactInfo(User.Identity.Name);");
                    ...
                    if (TypeName == "AspNetUser")
                    {
                        sb.AppendLine($@"            return await { TypeName }DBSLocalervice.Get{ TypeName.Replace("Local", "") }WithId(Id);");
                    }
                    else
                    {
                        sb.AppendLine($@"            return await { TypeName }DBService.Get{ TypeName }With{ TypeName.Replace("Local", "") }ID({ TypeName.Replace("Local", "") }ID);");
                    }
                    sb.AppendLine(@"        }");
