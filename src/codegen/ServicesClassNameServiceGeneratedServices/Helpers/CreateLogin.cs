...
namespace ServicesClassNameServiceGeneratedServices.Services
{
    public partial class ServicesClassNameServiceGeneratedService : IServicesClassNameServiceGeneratedService
    {
        private async Task<bool> CreateLogin(DLLTypeInfo dllTypeInto, string TypeName, string TypeNameLower, StringBuilder sb)
        {
            ...
            sb.AppendLine(@"                {");
            sb.AppendLine(@"                    return BadRequest(String.Format(CSSPCultureServicesRes.__CouldNotBeFound, CSSPCultureServicesRes.Email, loginModel.LoginEmail));");
            sb.AppendLine(@"                }");
            sb.AppendLine(@"");
            ...
            sb.AppendLine(@"                {");
            sb.AppendLine(@"                    return BadRequest(String.Format(CSSPCultureServicesRes.UnableToLoginAs_WithProvidedPassword, loginModel.LoginEmail));");
            sb.AppendLine(@"                }");
            sb.AppendLine(@"");
            ...
            sb.AppendLine(@"");
            sb.AppendLine(@"                    return contact;");
            sb.AppendLine(@"                }");
            sb.AppendLine(@"            }");
            sb.AppendLine(@"            catch (Exception ex)");
            sb.AppendLine(@"            {");
            sb.AppendLine(@"                return BadRequest(String.Format(CSSPCultureServicesRes.Error_, ex.Message));");
            sb.AppendLine(@"            }");
            sb.AppendLine(@"");
            sb.AppendLine(@"            return BadRequest(String.Format(CSSPCultureServicesRes.UnableToLoginAs_WithProvidedPassword, loginModel.LoginEmail));");
            sb.AppendLine(@"        }");
