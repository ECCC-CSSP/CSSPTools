...
{
    public class ServicesRepopulateTestDBService : IServicesRepopulateTestDBService
    {
        #region Variables
        #endregion Variables
        ...
        private CSSPDBContext dbTestDB { get; set; }
        ...
        private async Task<bool> Generate()
        {
            ...
            if (!CompareDBs(tableCSSPDBList, tableTestDBList)) return false;
            ...
            if (!CleanTestDB(tableTestDBList, TestDBConnectionString)) return false;
            ...
            if (!FillTestDB(tableTestDBList)) return false;
            ...
            if (!MakingSureEveryTableHasEnoughItemsInTestDB()) return false;
        ...
        private bool FillTestDB(List<Table> tableTestDBList)
        {
            ...
            if (!AddObject("TVItem", tvItemRainExceedance)) return false;
            if (!CorrectTVPath(tvItemRainExceedance, tvItemCanada)) return false;
            if (!AddMapInfo(tvItemRainExceedance, 374858, tvItemContactCharles.TVItemID)) return false;
        ...
        private bool AddMapInfo(TVItem NewTVItem, int OldTVItemID, int ContactTVItemID)
        {
            ...
            foreach (MapInfo mapInfo in mapInfoList)
            {
                ...
                mapInfo.MapInfoID = 0;
                using (TestDBContext dbTestDB = new TestDBContext())
                {
        ...
        private bool AddObject(string TypeName, object objTarget)
        {
            switch (TypeName)
            {
                ...
                case "MWQMSite":
                    ...
                    break;
                //case "MWQMSitePolSourceSite":
                //    {
                //        ((MWQMSitePolSourceSite)objTarget).MWQMSitePolSourceSiteID = 0;
                //        ((MWQMSitePolSourceSite)objTarget).LastUpdateContactTVItemID = 2;
                //        dbTestDB.MWQMSitePolSourceSites.Add((MWQMSitePolSourceSite)objTarget);
                //    }
                //    break;
        ...
        private bool MakingSureEveryTableHasEnoughItemsInTestDB()
        {
            ...
            //        for (int i = count; i < 10; i++)
            //        {
            //            MWQMSitePolSourceSite MWQMSitePolSourceSite = (from c in db.MWQMSitePolSourceSites select c).OrderByDescending(c => c.MWQMSitePolSourceSiteID).FirstOrDefault();
            //            try
            //            {
