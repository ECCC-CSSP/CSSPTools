...
namespace CSSPAzureAppTaskServices
{
    ...
    public partial class AzureAppTaskService : ControllerBase, IAzureAppTaskService
    {
        private async Task<bool> ValidateAzureAddOrModifyAppTaskModel(PostAppTaskModel postAppTaskModel)
        {
            ...
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._IsRequired, "DBCommand"), new[] { "DBCommand" }));
            }

            if (postAppTaskModel.AppTask.TVItemID == 0)
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._IsRequired, "TVItemID"), new[] { "TVItemID" }));
            }

            if (postAppTaskModel.AppTask.TVItemID2 == 0)
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._IsRequired, "TVItemID2"), new[] { "TVItemID2" }));
            }

            retStr = enums.EnumTypeOK(typeof(AppTaskCommandEnum), (int?)postAppTaskModel.AppTask.AppTaskCommand);
            if (!string.IsNullOrWhiteSpace(retStr))
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._IsRequired, "AppTaskCommand"), new[] { "AppTaskCommand" }));
            }

            retStr = enums.EnumTypeOK(typeof(AppTaskStatusEnum), (int?)postAppTaskModel.AppTask.AppTaskStatus);
            if (!string.IsNullOrWhiteSpace(retStr))
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._IsRequired, "AppTaskStatus"), new[] { "AppTaskStatus" }));
            }

            if (postAppTaskModel.AppTask.PercentCompleted < 0)
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._IsRequired, "PercentCompleted"), new[] { "PercentCompleted" }));
            }

            //if (string.IsNullOrWhiteSpace(appTaskModel.AppTask.Parameters))
            //{
            //    await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._IsRequired, "Parameters"), new[] { "Parameters" }));
            //}

            retStr = enums.EnumTypeOK(typeof(LanguageEnum), (int?)postAppTaskModel.AppTask.Language);
            if (!string.IsNullOrWhiteSpace(retStr))
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._IsRequired, "Language"), new[] { "Language" }));
            }

            if (postAppTaskModel.AppTask.StartDateTime_UTC.Year < 1980)
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._YearShouldBeBiggerThan_, "StartDateTime_UTC", "1979"), new[] { "StartDateTime_UTC" }));
            }
            ...
            if (postAppTaskModel.AppTask.EndDateTime_UTC != null)
            {
                ...
                {
                    await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._YearShouldBeBiggerThan_, "EndDateTime_UTC", "1979"), new[] { "EndDateTime_UTC" }));
                }
            }

            if (postAppTaskModel.AppTaskLanguageList.Count != 2)
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._ShouldBeEqualTo_, "appTaskModel.AppTaskLanguageList.Count", "2"), new[] { "" }));
            }

            if (!postAppTaskModel.AppTaskLanguageList.Where(c => c.Language == LanguageEnum.en).Any())
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes.CouldNotFind_With_Equal_, "AppTaskLanguage", "Language", "en"), new[] { "" }));
            }

            if (!postAppTaskModel.AppTaskLanguageList.Where(c => c.Language == LanguageEnum.fr).Any())
            {
                await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes.CouldNotFind_With_Equal_, "AppTaskLanguage", "Language", "fr"), new[] { "" }));
            }
            ...
            if (CSSPLogService.ValidationResultList.Count == 0)
            {
                ...
                foreach (LanguageEnum lang in new List<LanguageEnum>() { LanguageEnum.en, LanguageEnum.fr })
                {
                    ...
                    if (postAppTaskModel.AppTask.AppTaskID == 0)
                    {
                        ...
                        {
                            await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._ShouldBeEqualTo_, "AppTaskLanguageID", "0"), new[] { "AppTaskLanguageID" }));
                        }
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(postAppTaskModel.AppTaskLanguageList[langID].StatusText))
                    {
                        ...
                        {
                            await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._MaxLengthIs_, "StatusText", 250), new[] { "StatusText" }));
                        }
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(postAppTaskModel.AppTaskLanguageList[langID].ErrorText))
                    {
                        ...
                        {
                            await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._MaxLengthIs_, "ErrorText", 250), new[] { "ErrorText" }));
                        }
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(retStr))
                    {
                        await CSSPLogService.AppendError(new ValidationResult(string.Format(CSSPCultureServicesRes._IsRequired, "TranslationStatus"), new[] { "TranslationStatus" }));
                    }
                }
            }
