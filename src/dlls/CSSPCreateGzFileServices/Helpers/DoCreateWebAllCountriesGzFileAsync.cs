...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebAllCountriesGzFileAsync()
        {
            ...
            try
            {
                ...
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebAllCountries>(webAllCountries, $"{ WebTypeEnum.WebAllCountries }.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebAllCountries>(webAllCountries, $"{ WebTypeEnum.WebAllCountries }.gz")) return await Task.FromResult(false);
                }
            }
            catch (Exception ex)
            {
