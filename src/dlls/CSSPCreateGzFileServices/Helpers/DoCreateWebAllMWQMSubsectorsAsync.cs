...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebAllMWQMSubsectorsGzFileAsync()
        {
            ...
            try
            {
                ...
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebAllMWQMSubsectors>(webAllMWQMSubsectors, $"{ WebTypeEnum.WebAllMWQMSubsectors }.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebAllMWQMSubsectors>(webAllMWQMSubsectors, $"{ WebTypeEnum.WebAllMWQMSubsectors }.gz")) return await Task.FromResult(false);
                }
            }
            catch (Exception ex)
            {
