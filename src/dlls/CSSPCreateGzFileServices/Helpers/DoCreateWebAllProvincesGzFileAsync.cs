...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebAllProvincesGzFileAsync()
        {
            ...
            try
            {
                ...
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebAllProvinces>(webAllProvinces, $"{ WebTypeEnum.WebAllProvinces }.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebAllProvinces>(webAllProvinces, $"{ WebTypeEnum.WebAllProvinces }.gz")) return await Task.FromResult(false);
                }
            }
            catch (Exception ex)
            {
