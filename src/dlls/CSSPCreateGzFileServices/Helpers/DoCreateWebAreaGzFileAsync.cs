...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebAreaGzFileAsync(int AreaTVItemID)
        {
            ...
            try
            {
                ...
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebArea>(webArea, $"{ WebTypeEnum.WebArea }_{ AreaTVItemID }.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebArea>(webArea, $"{ WebTypeEnum.WebArea }_{ AreaTVItemID }.gz")) return await Task.FromResult(false);
                }
            }
            catch (Exception ex)
            {
