...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebMWQMSamples2021_2060GzFileAsync(int SubsectorTVItemID)
        {
            ...
            try
            {
                ...
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebMWQMSamples>(webMWQMSamples, $"{ WebTypeEnum.WebMWQMSamples2021_2060 }_{ SubsectorTVItemID }.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebMWQMSamples>(webMWQMSamples, $"{ WebTypeEnum.WebMWQMSamples2021_2060 }_{ SubsectorTVItemID }.gz")) return await Task.FromResult(false);
                }
            }
            catch (Exception ex)
            {
