...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebMonitoringOtherStatsProvinceGzFileAsync(int ProvinceTVItemID)
        {
            ...
            try
            {
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebMonitoringOtherStatsProvince>(webMonitoringOtherStatsProvince, $"{ WebTypeEnum.WebMonitoringOtherStatsProvince }_{tvItemProv.TVItemID}.gz")) return await Task .FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebMonitoringOtherStatsProvince>(webMonitoringOtherStatsProvince, $"{ WebTypeEnum.WebMonitoringOtherStatsProvince }_{tvItemProv.TVItemID}.gz")) return await Task .FromResult(false);
                }
