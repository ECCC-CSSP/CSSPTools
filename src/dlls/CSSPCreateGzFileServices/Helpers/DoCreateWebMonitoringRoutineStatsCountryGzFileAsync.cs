...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebMonitoringRoutineStatsCountryGzFileAsync(int CountryTVItemID)
        {
            ...
            try
            {
                ...
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebMonitoringRoutineStatsCountry>(webMonitoringRoutineStatsCountry, $"{ WebTypeEnum.WebMonitoringRoutineStatsCountry }_{tvItemCountry.TVItemID}.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebMonitoringRoutineStatsCountry>(webMonitoringRoutineStatsCountry, $"{ WebTypeEnum.WebMonitoringRoutineStatsCountry }_{tvItemCountry.TVItemID}.gz")) return await Task.FromResult(false);
                }
            }
            catch (Exception ex)
            {
