    ...
    public partial class CreateGzFileService : ControllerBase, ICreateGzFileService
    {
        private async Task<bool> DoCreateWebMonitoringRoutineStatsProvinceGzFile(int ProvinceTVItemID)
        {
            ...
            try
            {
                if (dbLocal != null)
                {
                    if (!await DoStoreLocal<WebMonitoringRoutineStatsProvince>(webMonitoringRoutineStatsProvince, $"{ WebTypeEnum.WebMonitoringRoutineStatsProvince }_{tvItemProv.TVItemID}.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStore<WebMonitoringRoutineStatsProvince>(webMonitoringRoutineStatsProvince, $"{ WebTypeEnum.WebMonitoringRoutineStatsProvince }_{tvItemProv.TVItemID}.gz")) return await Task.FromResult(false);
                }
