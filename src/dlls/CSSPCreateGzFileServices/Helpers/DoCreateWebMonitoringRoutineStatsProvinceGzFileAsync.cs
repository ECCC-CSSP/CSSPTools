...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebMonitoringRoutineStatsProvinceGzFileAsync(int ProvinceTVItemID)
        {
            ...
            try
            {
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebMonitoringRoutineStatsProvince>(webMonitoringRoutineStatsProvince, $"{ WebTypeEnum.WebMonitoringRoutineStatsProvince }_{tvItemProv.TVItemID}.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebMonitoringRoutineStatsProvince>(webMonitoringRoutineStatsProvince, $"{ WebTypeEnum.WebMonitoringRoutineStatsProvince }_{tvItemProv.TVItemID}.gz")) return await Task.FromResult(false);
                }
