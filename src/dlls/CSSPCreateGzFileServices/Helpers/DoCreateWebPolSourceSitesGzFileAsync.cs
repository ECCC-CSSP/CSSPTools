...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebPolSourceSitesGzFileAsync(int SubsectorTVItemID)
        {
            ...
            try
            {
                ...
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebPolSourceSites>(webPolSourceSites, $"{ WebTypeEnum.WebPolSourceSites }_{ SubsectorTVItemID }.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebPolSourceSites>(webPolSourceSites, $"{ WebTypeEnum.WebPolSourceSites }_{ SubsectorTVItemID }.gz")) return await Task.FromResult(false);
                }
            }
            catch (Exception ex)
            {
