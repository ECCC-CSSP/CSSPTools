...
namespace CSSPCreateGzFileServices
{
    ...
    {
        private async Task<bool> DoCreateWebTideSitesGzFileAsync(int ProvinceTVItemID)
        {
            ...
            try
            {
                ...
                if (dbLocal != null)
                {
                    if (!await DoStoreLocalAsync<WebTideSites>(webTideSites, $"{ WebTypeEnum.WebTideSites }_{ ProvinceTVItemID }.gz")) return await Task.FromResult(false);
                }
                else
                {
                    if (!await DoStoreAsync<WebTideSites>(webTideSites, $"{ WebTypeEnum.WebTideSites }_{ ProvinceTVItemID }.gz")) return await Task.FromResult(false);
                }
            }
            catch (Exception ex)
            {
