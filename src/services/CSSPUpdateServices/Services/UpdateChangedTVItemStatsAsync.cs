...
{
    public async Task<ActionResult<bool>> UpdateChangedTVItemStatsAsync(DateTime UpdateFromDate)
    {
        ...
        for (int i = 0, countStat = TVItemForChangedStatList.Count; i < countStat; i++)
        {
            ...
            List<TVTypeEnum> SubTVTypeList = GetSubTVTypeForTVItemStatAsync(TVItemForChangedStatList[i].TVType);
