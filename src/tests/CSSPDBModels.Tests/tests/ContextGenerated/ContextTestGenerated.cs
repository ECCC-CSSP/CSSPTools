...
namespace CSSPDBModels.Tests
{
    public partial class ContextTest
    {
        ...
        [InlineData("en-CA")]
        //[InlineData("fr-CA")]
        public async Task TestDBContext_Good_Test(string culture)
        {
            Assert.True(await Setup(culture));
            ...
            TestDBContext testDBContext2 = Provider.GetService<TestDBContext>();
            Assert.NotNull(testDBContext2);

            var Addresses = testDBContext2.Addresses;
            Assert.NotNull(Addresses);

            var AppErrLogs = testDBContext2.AppErrLogs;
            Assert.NotNull(AppErrLogs);

            var AppTasks = testDBContext2.AppTasks;
            Assert.NotNull(AppTasks);

            var AppTaskLanguages = testDBContext2.AppTaskLanguages;
            Assert.NotNull(AppTaskLanguages);

            var BoxModels = testDBContext2.BoxModels;
            Assert.NotNull(BoxModels);

            var BoxModelLanguages = testDBContext2.BoxModelLanguages;
            Assert.NotNull(BoxModelLanguages);

            var BoxModelResults = testDBContext2.BoxModelResults;
            Assert.NotNull(BoxModelResults);

            var Classifications = testDBContext2.Classifications;
            Assert.NotNull(Classifications);

            var ClimateDataValues = testDBContext2.ClimateDataValues;
            Assert.NotNull(ClimateDataValues);

            var ClimateSites = testDBContext2.ClimateSites;
            Assert.NotNull(ClimateSites);

            var CoCoRaHSSites = testDBContext2.CoCoRaHSSites;
            Assert.NotNull(CoCoRaHSSites);

            var CoCoRaHSValues = testDBContext2.CoCoRaHSValues;
            Assert.NotNull(CoCoRaHSValues);

            var Contacts = testDBContext2.Contacts;
            Assert.NotNull(Contacts);

            var ContactPreferences = testDBContext2.ContactPreferences;
            Assert.NotNull(ContactPreferences);

            var ContactShortcuts = testDBContext2.ContactShortcuts;
            Assert.NotNull(ContactShortcuts);

            var DocTemplates = testDBContext2.DocTemplates;
            Assert.NotNull(DocTemplates);

            var DrogueRuns = testDBContext2.DrogueRuns;
            Assert.NotNull(DrogueRuns);

            var DrogueRunPositions = testDBContext2.DrogueRunPositions;
            Assert.NotNull(DrogueRunPositions);

            var Emails = testDBContext2.Emails;
            Assert.NotNull(Emails);

            var EmailDistributionLists = testDBContext2.EmailDistributionLists;
            Assert.NotNull(EmailDistributionLists);

            var EmailDistributionListContacts = testDBContext2.EmailDistributionListContacts;
            Assert.NotNull(EmailDistributionListContacts);

            var EmailDistributionListContactLanguages = testDBContext2.EmailDistributionListContactLanguages;
            Assert.NotNull(EmailDistributionListContactLanguages);

            var EmailDistributionListLanguages = testDBContext2.EmailDistributionListLanguages;
            Assert.NotNull(EmailDistributionListLanguages);

            var HelpDocs = testDBContext2.HelpDocs;
            Assert.NotNull(HelpDocs);

            var HydrometricDataValues = testDBContext2.HydrometricDataValues;
            Assert.NotNull(HydrometricDataValues);

            var HydrometricSites = testDBContext2.HydrometricSites;
            Assert.NotNull(HydrometricSites);

            var Infrastructures = testDBContext2.Infrastructures;
            Assert.NotNull(Infrastructures);

            var InfrastructureLanguages = testDBContext2.InfrastructureLanguages;
            Assert.NotNull(InfrastructureLanguages);

            var LabSheets = testDBContext2.LabSheets;
            Assert.NotNull(LabSheets);

            var LabSheetDetails = testDBContext2.LabSheetDetails;
            Assert.NotNull(LabSheetDetails);

            var LabSheetTubeMPNDetails = testDBContext2.LabSheetTubeMPNDetails;
            Assert.NotNull(LabSheetTubeMPNDetails);

            var Logs = testDBContext2.Logs;
            Assert.NotNull(Logs);

            var MapInfos = testDBContext2.MapInfos;
            Assert.NotNull(MapInfos);

            var MapInfoPoints = testDBContext2.MapInfoPoints;
            Assert.NotNull(MapInfoPoints);

            var MikeBoundaryConditions = testDBContext2.MikeBoundaryConditions;
            Assert.NotNull(MikeBoundaryConditions);

            var MikeScenarios = testDBContext2.MikeScenarios;
            Assert.NotNull(MikeScenarios);

            var MikeScenarioResults = testDBContext2.MikeScenarioResults;
            Assert.NotNull(MikeScenarioResults);

            var MikeSources = testDBContext2.MikeSources;
            Assert.NotNull(MikeSources);

            var MikeSourceStartEnds = testDBContext2.MikeSourceStartEnds;
            Assert.NotNull(MikeSourceStartEnds);

            var MWQMAnalysisReportParameters = testDBContext2.MWQMAnalysisReportParameters;
            Assert.NotNull(MWQMAnalysisReportParameters);

            var MWQMLookupMPNs = testDBContext2.MWQMLookupMPNs;
            Assert.NotNull(MWQMLookupMPNs);

            var MWQMRuns = testDBContext2.MWQMRuns;
            Assert.NotNull(MWQMRuns);

            var MWQMRunLanguages = testDBContext2.MWQMRunLanguages;
            Assert.NotNull(MWQMRunLanguages);

            var MWQMSamples = testDBContext2.MWQMSamples;
            Assert.NotNull(MWQMSamples);

            var MWQMSampleLanguages = testDBContext2.MWQMSampleLanguages;
            Assert.NotNull(MWQMSampleLanguages);

            var MWQMSites = testDBContext2.MWQMSites;
            Assert.NotNull(MWQMSites);

            var MWQMSiteStartEndDates = testDBContext2.MWQMSiteStartEndDates;
            Assert.NotNull(MWQMSiteStartEndDates);

            var MWQMSubsectors = testDBContext2.MWQMSubsectors;
            Assert.NotNull(MWQMSubsectors);

            var MWQMSubsectorLanguages = testDBContext2.MWQMSubsectorLanguages;
            Assert.NotNull(MWQMSubsectorLanguages);

            var PolSourceGroupings = testDBContext2.PolSourceGroupings;
            Assert.NotNull(PolSourceGroupings);

            var PolSourceGroupingLanguages = testDBContext2.PolSourceGroupingLanguages;
            Assert.NotNull(PolSourceGroupingLanguages);

            var PolSourceObservations = testDBContext2.PolSourceObservations;
            Assert.NotNull(PolSourceObservations);

            var PolSourceObservationIssues = testDBContext2.PolSourceObservationIssues;
            Assert.NotNull(PolSourceObservationIssues);

            var PolSourceSites = testDBContext2.PolSourceSites;
            Assert.NotNull(PolSourceSites);

            var PolSourceSiteEffects = testDBContext2.PolSourceSiteEffects;
            Assert.NotNull(PolSourceSiteEffects);

            var PolSourceSiteEffectTerms = testDBContext2.PolSourceSiteEffectTerms;
            Assert.NotNull(PolSourceSiteEffectTerms);

            var RainExceedances = testDBContext2.RainExceedances;
            Assert.NotNull(RainExceedances);

            var RainExceedanceClimateSites = testDBContext2.RainExceedanceClimateSites;
            Assert.NotNull(RainExceedanceClimateSites);

            var RatingCurves = testDBContext2.RatingCurves;
            Assert.NotNull(RatingCurves);

            var RatingCurveValues = testDBContext2.RatingCurveValues;
            Assert.NotNull(RatingCurveValues);

            var ReportSections = testDBContext2.ReportSections;
            Assert.NotNull(ReportSections);

            var ReportTypes = testDBContext2.ReportTypes;
            Assert.NotNull(ReportTypes);

            var ResetPasswords = testDBContext2.ResetPasswords;
            Assert.NotNull(ResetPasswords);

            var SamplingPlans = testDBContext2.SamplingPlans;
            Assert.NotNull(SamplingPlans);

            var SamplingPlanEmails = testDBContext2.SamplingPlanEmails;
            Assert.NotNull(SamplingPlanEmails);

            var SamplingPlanSubsectors = testDBContext2.SamplingPlanSubsectors;
            Assert.NotNull(SamplingPlanSubsectors);

            var SamplingPlanSubsectorSites = testDBContext2.SamplingPlanSubsectorSites;
            Assert.NotNull(SamplingPlanSubsectorSites);

            var Spills = testDBContext2.Spills;
            Assert.NotNull(Spills);

            var SpillLanguages = testDBContext2.SpillLanguages;
            Assert.NotNull(SpillLanguages);

            var Tels = testDBContext2.Tels;
            Assert.NotNull(Tels);

            var TideDataValues = testDBContext2.TideDataValues;
            Assert.NotNull(TideDataValues);

            var TideLocations = testDBContext2.TideLocations;
            Assert.NotNull(TideLocations);

            var TideSites = testDBContext2.TideSites;
            Assert.NotNull(TideSites);

            var TVFiles = testDBContext2.TVFiles;
            Assert.NotNull(TVFiles);

            var TVFileLanguages = testDBContext2.TVFileLanguages;
            Assert.NotNull(TVFileLanguages);

            var TVItems = testDBContext2.TVItems;
            Assert.NotNull(TVItems);

            var TVItemLanguages = testDBContext2.TVItemLanguages;
            Assert.NotNull(TVItemLanguages);

            var TVItemLinks = testDBContext2.TVItemLinks;
            Assert.NotNull(TVItemLinks);

            var TVItemStats = testDBContext2.TVItemStats;
            Assert.NotNull(TVItemStats);

            var TVItemUserAuthorizations = testDBContext2.TVItemUserAuthorizations;
            Assert.NotNull(TVItemUserAuthorizations);

            var TVTypeUserAuthorizations = testDBContext2.TVTypeUserAuthorizations;
            Assert.NotNull(TVTypeUserAuthorizations);

            var UseOfSites = testDBContext2.UseOfSites;
            Assert.NotNull(UseOfSites);

            var VPAmbients = testDBContext2.VPAmbients;
            Assert.NotNull(VPAmbients);

            var VPResults = testDBContext2.VPResults;
            Assert.NotNull(VPResults);

            var VPScenarios = testDBContext2.VPScenarios;
            Assert.NotNull(VPScenarios);

            var VPScenarioLanguages = testDBContext2.VPScenarioLanguages;
            Assert.NotNull(VPScenarioLanguages);
