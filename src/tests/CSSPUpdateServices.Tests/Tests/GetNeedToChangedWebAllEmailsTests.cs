...
namespace UpdateServices.Tests
{
    public partial class UpdateServiceTests
    {
        ...
        [InlineData("en-CA")]
        //[InlineData("fr-CA")]
        public async Task UpdateService_GetNeedToChangedWebAllEmails_HasEmail_Good_Test(string culture)
        {
            Assert.True(await Setup(culture));

            DateTime LastUpdateDate_UTC = GetLastUpdateDate_UTC_Email().AddDays(-1);

            bool exist = await CSSPUpdateService.GetNeedToChangedWebAllEmails(LastUpdateDate_UTC);
            Assert.True(exist);
        }
        [Theory]
        [InlineData("en-CA")]
        //[InlineData("fr-CA")]
        public async Task UpdateService_GetNeedToChangedWebAllEmails_NoEmail_Good_Test(string culture)
        {
            Assert.True(await Setup(culture));
