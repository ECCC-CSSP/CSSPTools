...
namespace UpdateServices.Tests
{
    public partial class UpdateServiceTests
    {
        ...
        [InlineData("en-CA")]
        //[InlineData("fr-CA")]
        public async Task UpdateService_GetTVItemListCountryOfChangedRainExceedance_HasRainExceedance_Good_Test(string culture)
        {
            Assert.True(await Setup(culture));

            DateTime LastUpdateDate_UTC = GetLastUpdateDate_UTC_RainExceedance().AddDays(-1);

            List<int> ProvinceTVItemIDList = await CSSPUpdateService.GetTVItemIDListCountryOfChangedRainExceedance(LastUpdateDate_UTC);
            Assert.True(ProvinceTVItemIDList.Count > 0);
        }
        [Theory]
        [InlineData("en-CA")]
        //[InlineData("fr-CA")]
        public async Task UpdateService_GetTVItemListCountryOfChangedRainExceedance_NoRainExceedance_Good_Test(string culture)
        {
            Assert.True(await Setup(culture));

            DateTime LastUpdateDate_UTC = GetLastUpdateDate_UTC_RainExceedance().AddDays(1);

            List<int> CountryTVItemIDList = await CSSPUpdateService.GetTVItemIDListCountryOfChangedRainExceedance(LastUpdateDate_UTC);
