...
namespace CSSPDesktop
{
    public partial class CSSPDesktopForm : Form
    {
        ...
        private async Task<bool> Setup()
        {
            ...
            FileInfo fi = new FileInfo(CSSPDesktopService.CSSPWebAPIsExeFullPath);
            if (!fi.Exists)
            {
                richTextBoxStatus.Text = $"Could not find file to run CSSPWebAPIs [{ fi.FullName }]";
                return await Task.FromResult(false);
            }

            DirectoryInfo di = new DirectoryInfo(CSSPDesktopService.HelpPath);
            if (!di.Exists)
            {
                richTextBoxStatus.Text = $"Could not find help path [{ di.FullName }]";
                return await Task.FromResult(false);
            }
