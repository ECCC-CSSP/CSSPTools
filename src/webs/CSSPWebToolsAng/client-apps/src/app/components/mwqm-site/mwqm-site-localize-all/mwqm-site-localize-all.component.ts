...
@Component({
  selector: 'app-mwqm-site-localize-all',
  templateUrl: './mwqm-site-localize-all.component.html',
  styleUrls: ['./mwqm-site-localize-all.component.css']
})
export class MWQMSiteLocalizeAllComponent implements OnInit, OnDestroy {
  ...
  LocalizeAllFiles() {
    ...
    for (let i = 0, countI = this.appLoadedService.WebMWQMSites?.MWQMSiteModelList?.length; i < countI; i++) {
      for (let j = 0, countJ = this.appLoadedService.WebMWQMSites?.MWQMSiteModelList[i]?.TVFileModelList?.length; j < countJ; j++) {
        TVFileModelList.push(this.appLoadedService.WebMWQMSites?.MWQMSiteModelList[i]?.TVFileModelList[j]);
      }
    }

    this.fileLocalizeAllAzureFileService.AddTVFileModelList(TVFileModelList);
    this.fileLocalizeAllAzureFileService.LocalizeAllAzureFile(this.tvType.MWQMSite);
  }
  ...
  GetLocalizeStats(): string {
    ...
    for (let i = 0, countI = this.appLoadedService.WebMWQMSites?.MWQMSiteModelList?.length; i < countI; i++) {
      for (let j = 0, countJ = this.appLoadedService.WebMWQMSites?.MWQMSiteModelList[i]?.TVFileModelList?.length; j < countJ; j++) {
        FilesCount += 1;
        FileTotalSize += this.appLoadedService.WebMWQMSites?.MWQMSiteModelList[i]?.TVFileModelList[j].TVFile.FileSize_kb;
        if (this.appLoadedService.WebMWQMSites?.MWQMSiteModelList[i]?.TVFileModelList[j].IsLocalized) {
          FileLocalizedCount += 1;
          FileLocalizedSize += this.appLoadedService.WebMWQMSites?.MWQMSiteModelList[i]?.TVFileModelList[j].TVFile.FileSize_kb;
        }
      }
    }
