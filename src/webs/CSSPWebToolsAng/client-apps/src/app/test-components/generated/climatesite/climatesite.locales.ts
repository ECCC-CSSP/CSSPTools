/*
 * Auto generated from C:\CSSPTools\src\codegen\AngularComponentsGenerated\bin\Debug\netcoreapp3.1\AngularComponentsGenerated.exe
 *
 * Do not edit this file.
 *
 */

import { ClimateSiteTextModel }
from './climatesite.models';
import { ClimateSiteService }
from './climatesite.service';

export function LoadLocalesClimateSiteText(climatesiteService: ClimateSiteService) {
    let climatesiteTextModel: ClimateSiteTextModel = {
        Title: 'The title',
}

    if ($localize.locale === 'fr-CA') {
        climatesiteTextModel.Title = 'Le Titre';
    }

    climatesiteService.UpdateClimateSiteText(climatesiteTextModel);
}
