/*
 * Auto generated from C:\CSSPTools\src\codegen\AngularComponentsGenerated\bin\Debug\netcoreapp3.1\AngularComponentsGenerated.exe
 *
 * Do not edit this file.
 *
 */

import { TVItemLinkTextModel }
from './tvitemlink.models';
import { TVItemLinkService }
from './tvitemlink.service';

export function LoadLocalesTVItemLinkText(tvitemlinkService: TVItemLinkService) {
    let tvitemlinkTextModel: TVItemLinkTextModel = {
        Title: 'The title',
}

    if ($localize.locale === 'fr-CA') {
        tvitemlinkTextModel.Title = 'Le Titre';
    }

    tvitemlinkService.UpdateTVItemLinkText(tvitemlinkTextModel);
}
