/*
 * Auto generated from C:\CSSPTools\src\codegen\AngularComponentsGenerated\bin\Debug\netcoreapp3.1\AngularComponentsGenerated.exe
 *
 * Do not edit this file.
 *
 */

import { TVItemStatTextModel }
from './tvitemstat.models';
import { TVItemStatService }
from './tvitemstat.service';

export function LoadLocalesTVItemStatText(tvitemstatService: TVItemStatService) {
    let tvitemstatTextModel: TVItemStatTextModel = {
        Title: 'The title',
}

    if ($localize.locale === 'fr-CA') {
        tvitemstatTextModel.Title = 'Le Titre';
    }

    tvitemstatService.UpdateTVItemStatText(tvitemstatTextModel);
}
